...
func (r UserRepositoryPg) Create(user entity.User) error {
	...
	_, err := r.db.Exec(
		`INSERT INTO users (first_name, last_name, email, birthday, username, password) 
			VALUES ($1, $2, $3, $4, $5, crypt('$6', gen_salt('bf'))`,
		user.FirstName, user.LastName, user.Email, user.BirthDay, user.Username, user.Passwd,
	)
	if err != nil {
		return err
	}
